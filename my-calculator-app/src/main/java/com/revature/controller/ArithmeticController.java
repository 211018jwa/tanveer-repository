...
public Handler subtract = (ctx) -> {
		...
		// String + double -> String + String = String
		ctx.result(arithmeticService.doSubtraction(number1String, number2String));// Because number1 and number2 are just Strings, this will do String concatentation and not our actual adding of numbers
	};
public Handler multiply = (ctx) -> {
		...
		// String + double -> String + String = String
		ctx.result(arithmeticService.doMultiplication(number1String, number2String));// Because number1 and number2 are just Strings, this will do String concatentation and not our actual adding of numbers
	};
public Handler divison = (ctx) -> {
		...
		// String + double -> String + String = String
		ctx.result(arithmeticService.doDivide(number1String, number2String));// Because number1 and number2 are just Strings, this will do String concatentation and not our actual adding of numbers
	};
	...
	public void registerEndpoint(Javalin app) {
		...
		app.post("/divison", divison);// We are mapping the add lambda, which will be invoked whenever a client sends an HTTP POST request to "/add"
	}
